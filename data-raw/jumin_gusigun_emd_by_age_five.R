...
emd_demo_year_by_five_raw <- gusigun_code %>%
  distinct(.) %>%
...
emd_demo_year_by_five_raw %>%
  write_rds("data-raw/emd_demo_year_by_five_raw.rds")

# 3. 데이터 정제 -------------

emd_demo_year_by_five_raw <-
  read_rds("data-raw/emd_demo_year_by_five_raw.rds")
  # read_rds("data-raw/emd_demo_year_raw.rds")

emd_demo_year_by_five_tbl <- emd_demo_year_by_five_raw %>%
  ### NULL 값 처리
...
usethis::use_data(emd_demo_year_by_five_tbl, overwrite = TRUE)

# 5. 정합성 검증 -------------

test_that("행안부 인구통계 - 성별, 연령별, 읍면동별", {

  emd_demo_summary_tbl <- emd_demo_year_by_five_tbl %>%
    pivot_longer(cols = starts_with("x")) %>%
